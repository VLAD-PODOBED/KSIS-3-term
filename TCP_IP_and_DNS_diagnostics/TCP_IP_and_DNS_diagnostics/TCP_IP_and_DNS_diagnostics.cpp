...
IP_OPTION_INFORMATION option = { 255, 0, 0, 0, 0 }; //64 бита
...
void Ping(const char* cHost,
	...
	PICMP_ECHO_REPLY pIpe =    //писывает данные, возвращаемые в ответ на эхо-запрос IPv4.
	...
	for (unsigned int c = 0; c < RequestCount; c++) {
		...
		else {  //В случае ошибки считаем число потерянных пакетов и выводим 
			//код ошибки :
			if (pIpe->Status) {
				LostPacketsCount++;
				switch (pIpe->Status) {
